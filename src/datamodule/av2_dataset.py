...
def sept_collate_fn_lane_candidate(batch):
    ...
    for key in [
        ...
        data[key+'_candidate'] = pad_sequence([b[key][b['lane_candidate'].bool()] for b in batch], batch_first=True)
    ...
    for key in ["lane_padding_mask"]:
        data[key+'_candidate'] = pad_sequence(
            [b[key][b['lane_candidate'].bool()] for b in batch], batch_first=True, padding_value=True
        )

    data["x_key_padding_mask"] = data["x_padding_mask"].all(-1)
    data["lane_key_padding_mask"] = data["lane_padding_mask"].all(-1)
    data["lane_key_padding_mask_candidate"] = data["lane_padding_mask_candidate"].all(-1)
    data["num_actors"] = (~data["x_key_padding_mask"]).sum(-1)
    data["num_lanes"] = (~data["lane_key_padding_mask"]).sum(-1)
    data["num_lanes_candidate"] = (~data["lane_key_padding_mask_candidate"]).sum(-1)
