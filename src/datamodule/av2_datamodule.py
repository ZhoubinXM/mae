...
if __name__ == "__main__":
    ...
    def filter_trajs_end_pts(train_loader, train_trajs, train_end_pts, train_attrs):
    ...
    def cluster_trajs_end_pts(trajs, end_pts, pre_fix, k=6, viz=True):
        ...
        for i in range(k):
            end_pts_anchors[i] = np.mean(end_pts[end_pts_clustering.labels_ == i], axis=0)
        save_file = os.path.join(anchor_save_path, pre_fix + f"_anchor_end_pts_{k}.npy")
        print(f"[INFO]: Save path: {save_file}")
        np.save(save_file, end_pts_anchors)

        ds_size = trajs.shape[0]
        clustering = KMeans(n_clusters=k, verbose=1).fit(trajs.reshape((ds_size, -1)))
        anchors = np.zeros((k, 60, 2))
        for i in range(k):
            anchors[i] = np.mean(trajs[clustering.labels_ == i], axis=0)
        save_file = os.path.join(anchor_save_path, pre_fix + f"_anchor_trajs_{k}.npy")
        print(f"[INFO]: Save path: {save_file}")
        ...
        if viz:
            ...
            save_file = os.path.join(anchor_save_path, pre_fix + f"clusters_{k}.png")
            plt.savefig(save_file)

    train_val_data = Av2DataModule(data_root=data_root,
                  data_folder="multiagent-baseline-norm-200",
                  num_workers=12,
    ...
    for i in range(3):
        cls_mask = (train_attrs == i)
        cluster_trajs_end_pts(train_trajs[cls_mask], train_end_pts[cls_mask], pre_fix=pre_fix+f"_{TYPE_MAP[i]}")
    ...
    for i in range(3):
        cls_mask = (val_attrs == i)
        cluster_trajs_end_pts(val_trajs[cls_mask], val_end_pts[cls_mask], pre_fix=pre_fix+f"_{TYPE_MAP[i]}")
    ...
    for i in range(3):
        cls_mask = (test_attrs == i)
        cluster_trajs_end_pts(test_trajs[cls_mask], test_end_pts[cls_mask], pre_fix=pre_fix+f"_{TYPE_MAP[i]}")

    # ============== All ================
    all_trajs = np.concatenate([train_trajs, val_trajs, test_trajs], axis=0)
    all_end_pts = np.concatenate([train_end_pts, val_end_pts, test_end_pts], axis=0)
    ...
    for i in range(3):
        cls_mask = (all_attrs == i)
        cluster_trajs_end_pts(all_trajs[cls_mask], all_end_pts[cls_mask], pre_fix=pre_fix+f"_{TYPE_MAP[i]}")
