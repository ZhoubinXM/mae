...
class ModelMultiAgentMAE(nn.Module):
    ...
    def __init__(
        ...
        self.tempo_glb_net = nn.ModuleList(
            Block(
                dim=embed_dim,
                num_heads=num_heads,
                attn_drop=0.2,
        ...
        self.road_self_net = nn.ModuleList(
            Block(
                dim=embed_dim,
                num_heads=num_heads,
                attn_drop=0.2,
    ...
    def forward(self, data):
        ...
        for i in range(0, len(self.encoder), 2):
            ...
            # actor_feat = actor_feat + traj_pos_embed
            actor_feat = self.tempo_glb_net[i // 2](
                actor_feat,
                key_padding_mask=hist_padding_mask[~hist_feat_key_padding],
            )
            ...
            actor_feat_tmp = torch.zeros(B * N,
                                         ...
                                         device=actor_feat.device)
            actor_feat_tmp[~hist_feat_key_padding] = actor_feat
            actor_feat = actor_feat_tmp.reshape(B, N, T, actor_feat.shape[-1])
            ...
            actor_feat = actor_feat_tmp.reshape(B, T, N, actor_feat.shape[-1])
