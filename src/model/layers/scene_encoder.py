...
class SceneEncoder(nn.Module):
    ...
    def forward(self, data: dict, agent_feat: torch.Tensor,
                lane_feat: torch.Tensor, x_pose_embed, lane_pose_embed):
        ...
        scene_pose_embed = torch.cat([x_pose_embed, lane_pose_embed], dim=1)

        for blk in self.spa_net:
            scene_feat = blk(scene_feat + scene_pose_embed,
                             scene_feat + scene_pose_embed,
                             scene_feat,
                             key_padding_mask=scene_padding_mask)

        # scene_feat = self.scene_norm(scene_feat)

        return scene_feat, scene_pose_embed
