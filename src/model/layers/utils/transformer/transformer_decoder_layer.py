...
class TransformerDecoderLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if not rm_self_attn_decoder:
            ...
            if self.use_local_attn:
                ...
                self.cross_attn = MultiheadAttention(d_model * 2,
                                                     nhead,
                                                     dropout=dropout,
                                                     vdim=d_model,
                                                     batch_first=True,
    ...
    def forward(
        ...
        if memory is not None:
            _, _, M, _ = memory.shape
        # ========== Begin of Self-Attention =============
        if not self.rm_self_attn_decoder:
            # Mutually-Guided Intention Query
            ...
            if self.use_local_attn:
                ...
                query_pos = query_pos.permute(1, 0, 2).contiguous().view(
                    -1, n_model)  # (B * num_q, C)
                query_sine_embed = query_sine_embed.permute(
                    1, 0, 2).contiguous().view(-1, n_model)  # (B * num_q, C)
                tgt = tgt.permute(1, 0, 2).contiguous().view(
                    -1, n_model)  # (B * num_q, C)

            # ========== Begin of Cross-Attention =============
            # Apply projections here
            q_content = self.ca_qcontent_proj(tgt)  # [B, N*K, D]
            ...
            if self.use_local_attn and memory_valid_mask is not None:
                ...
                k_content = memory.new_zeros(memory.shape[0],
                                             k_content_valid.shape[-1])
                ...
                k_content = self.ca_kcontent_proj(memory)  # [B, N*K, M, D]
                v = self.ca_v_proj(memory)  # [B, N*K, M, D]
                k_pos = self.ca_kpos_proj(pos)  # [B, N*K, M, D]

            # For the first decoder layer, we concatenate the positional embedding predicted from
            # the object query (the positional embedding) into the original query (key) in DETR.
            if is_first or self.keep_query_pos:
                q_pos = self.ca_qpos_proj(query_pos)
                q = q_content + q_pos  # [B, N*K, D]
                k = k_content + k_pos  # [B, N*K, M, D]
            else:
                q = q_content
                k = k_content
            ...
            if self.use_local_attn:
                ...
                q = torch.cat([q, query_sine_embed],
                              dim=-1).view(num_q_all, n_model * 2)

                k = k.view(num_k_all, self.nhead, n_model // self.nhead)
                k_pos = k_pos.view(num_k_all, self.nhead,
                                   n_model // self.nhead)
                ...
                q = q.view(B, NK, self.nhead, D // self.nhead)
                query_sine_embed = query_sine_embed.view(
                    B, NK, self.nhead, D // self.nhead)
                # [B*N*A, 1, 2*D]
                q = torch.cat([q, query_sine_embed],
                              dim=3).view(B * NK, 1, D * 2)

                k = k.view(B * NK, M, self.nhead, D // self.nhead)
                k_pos = k_pos.view(B * NK, M, self.nhead, D // self.nhead)
                k = torch.cat([k, k_pos], dim=3).view(B * NK, M, D * 2)
                v = v.reshape(B * NK, M, D)
