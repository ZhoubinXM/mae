...
class LaneEncoder(nn.Module):
    ...
    def forward(self, data: dict):
        ...
        lane_actor_feat = lane_actor_feat_tmp.reshape(B, M, L, -1)

        lane_centers = data["lane_positions"].to(torch.float32)
        ...
        lane_angles = torch.stack(
            [torch.cos(lane_angles),
             torch.sin(lane_angles)], dim=-1).unsqueeze(2).repeat(1, 1, L, 1)
