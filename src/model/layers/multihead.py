...
def _scaled_dot_product_attention(q,
    ...
    if position_bias is not None:
        ...
        q2p_indices = (torch.arange(Nt).unsqueeze(0).unsqueeze(
            2) * Nt + torch.arange(Ns)).to(q2p.device)
        q2p = q2p.gather(2, q2p_indices.expand(B, Nt, Ns))
        attn += q2p
...
class myMultiheadAttention(nn.Module):
    ...
    def forward(
        ...
        if position_bias is not None:
            position_bias[0] = position_bias[0].reshape(
                batch_size, tgt_len, src_len, self.nhead,
                self.head_dim).permute(0, 3, 1, 2,
                                       4).reshape(batch_size * self.nhead,
                                                  tgt_len, src_len,
                                                  self.head_dim)
            position_bias[1] = position_bias[1].reshape(
                batch_size, tgt_len, src_len, self.nhead,
                self.head_dim).permute(0, 3, 1, 2,
                                       4).reshape(batch_size * self.nhead,
                                                  tgt_len, src_len,
