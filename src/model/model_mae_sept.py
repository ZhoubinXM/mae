...
class ModelMAESEPT(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.hist_embed = AgentEmbeddingLayer(4,
                                              embed_dim // 4,
        ...
        self.cross_attender = nn.ModuleList([
            CrossAttenderBlock(embed_dim,
                               ...
                               kdim=embed_dim,
                               vdim=embed_dim,
                               post_norm=True) for _ in range(3)
        ])

        self.decoder = MultimodalDecoder(embed_dim,
                                         future_steps,
                                         hidden_dim=256)
        self.dense_predictor = nn.Sequential(nn.Linear(embed_dim, 256),
                                             nn.ReLU(),
    ...
    def forward(self, data):
        ...
        actor_feat = self.hist_embed(hist_feat[~hist_feat_key_padding].permute(
            0, 2, 1).contiguous())
        ...
        lane_normalized = data["lane_positions"] - data[
            "lane_centers"].unsqueeze(-2)
        lane_normalized = torch.cat(
            [lane_normalized, ~lane_padding_mask[..., None]], dim=-1)
        B, M, L, D = lane_normalized.shape
        lane_feat = self.lane_embed(
            lane_normalized.view(-1, L, D).contiguous())
        ...
        x_encoder = self.norm(x_encoder)  # [B, A+R, D=128]
        
        x_encoder = x_encoder + pos_embed
        traj_query = self.agent_traj_query.repeat(x_encoder.size(0), 1,
                                                  1)  # [B,6,128]
        for blk in self.cross_attender:
            traj_query = blk(traj_query, x_encoder, x_encoder, key_padding_mask=key_padding_mask)

        x_agent = traj_query
        # x_agent = x_encoder[:, 0]  # first is predicted agent
        # change there use cross attender.
